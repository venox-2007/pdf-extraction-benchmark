"""Run the FINAL SROIE benchmark on the curated 20-document corpus.

Orchestrates all five extractors (PyMuPDF, OpenDataLoader, Tesseract,
PaddleOCR, Docling) over `data/final_benchmark/sroie/img/` and scores each
one against ground truth using the existing FUNSD CER/WER/Token-F1 metrics
(`pdf_extraction_benchmark.benchmarks.funsd.metrics`, unmodified) — the same
text-ground-truth methodology already used for the final FUNSD benchmark,
applied here to SROIE's ICDAR box-transcription ground truth
(`data/final_benchmark/sroie/box/*.txt`) instead of FUNSD's JSON annotations.

No existing extractor, metric, or benchmark module is modified. Extractors
run strictly sequentially in the order PyMuPDF -> OpenDataLoader -> Tesseract
-> PaddleOCR -> Docling, one fully completing across all 20 docs before the
next starts, so PaddleOCR and Docling are never loaded together.
"""
...
# scripts/run_final_funsd_benchmark.py.
...
IMG_DIR = ROOT / "data" / "final_benchmark" / "sroie" / "img"
BOX_DIR = ROOT / "data" / "final_benchmark" / "sroie" / "box"
OUT_DIR = ROOT / "outputs" / "benchmark_results" / "final_60" / "sroie"
...
# approach already used by the final FUNSD/RVL-CDIP scripts and the UI).
...
def _extract_ground_truth_text(box_path: Path) -> str:
    """Parse ICDAR/SROIE box-transcription lines into ground-truth text.

    Each line is `x1,y1,x2,y2,x3,y3,x4,y4,transcription` — splitting on the
    first 8 commas preserves any commas embedded in the transcription text.
    """
    raw_text = box_path.read_text(encoding="utf-8", errors="ignore")
    segments: list[str] = []
    for line in raw_text.splitlines():
        ...
        if not line:
            continue
        parts = line.split(",", 8)
        transcription = parts[8] if len(parts) >= 9 else parts[-1]
...
def _collect_documents() -> list[tuple[str, Path, Path]]:
    ...
    for image_path in images:
        box_path = BOX_DIR / f"{image_path.stem}.txt"
        if box_path.exists():
            documents.append((image_path.stem, image_path, box_path))
    if not documents:
        raise FileNotFoundError(f"No matching SROIE image/box pairs found in {IMG_DIR}")
    return documents
...
def _run_extractor(
    ...
    for document_id, image_path, _box_path in documents:
...
def _write_csv(all_results: list[DocResult]) -> Path:
    csv_path = OUT_DIR / "sroie_summary.csv"
...
def _write_json(
    ...
    json_path = OUT_DIR / "sroie_summary.json"
    payload = {
...
def _write_observations(
    ...
    lines.append("")
    lines.append(
        "- Ground truth is built by concatenating each receipt's box-transcription "
        "lines in file order, normalized identically to the FUNSD methodology "
        "(whitespace-collapsed, newline-joined)."
    )
    lines.append(
        "- PyMuPDF and OpenDataLoader require PDF input; each `.jpg` receipt is "
        "wrapped into a single-page PDF via fitz before extraction. OpenDataLoader "
        "ran in its default Java-only mode (no hybrid OCR backend)."
    )
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return md_path
...
def main() -> None:
    ...
    print(f"Collected {len(documents)} documents from {IMG_DIR}")

    ground_truths = {
        document_id: _normalize_text(_extract_ground_truth_text(box_path))
        for document_id, _image_path, box_path in documents
    }
